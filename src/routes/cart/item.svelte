<script>
    ...
    $:{
        if(parent){
            console.log(cost);
            // updateCart($cartItems.set(parent,cost))
        }
    }
...
<div class="md:grid card !border-[1px] p-2 prose-base w-full place-content-center m-2" bind:this={parent}>
    ...
    <div class="dark:child:text-white flex flex-col justify-evenly pl-2 child:!m-0">
        <h6 class="font-semibold text-ellipsis whitespace-nowrap overflow-hidden max-w-[20ch]">Apple Watch Series 7 GPS, Aluminium Case, Starlight Sport</h6>
        <p class="!text-gray-400">Electronics</p>
